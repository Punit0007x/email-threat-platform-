...
export default function CustodyReportPanel({ data }) {
  ...
  const getDomainAgeBadge = () => {
    ...
    if (days < 30) {
      return (
        <span className="px-2 py-0.5 rounded text-[10px] font-bold font-mono bg-[#ff4757]/15 text-[#d63031] border border-[#ff4757]/30">
    ...
    <div className="panel-chassis p-6 sm:p-8 space-y-6 relative overflow-hidden">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-[#d1d9e6] pb-4 px-2">
        <div className="flex items-center space-x-3.5">
          <div className="p-3 bg-[#e0e5ec] text-[#10b981] rounded-2xl shadow-[var(--shadow-card)] border border-white/70">
          ...
          <div>
            ...
            <p className="text-xs text-[#4a5568] font-mono">Cryptographic evidence preservation & legal reporting</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3 text-xs font-mono">
        <div className="slot-recessed p-3.5 rounded-xl">
          <span className="text-[#4a5568] font-bold block mb-1 text-[10px] uppercase">EVIDENCE ID</span>
          <span className="text-[#2d3436] font-bold">{custody.evidence_id}</span>
        </div>
        <div className="slot-recessed p-3.5 rounded-xl overflow-hidden">
          <span className="text-[#4a5568] font-bold block mb-1 text-[10px] uppercase">SHA-256 DIGEST</span>
          <span className="text-[#7048e8] truncate block font-bold">{custody.sha256}</span>
        </div>
        <div className="slot-recessed p-3.5 rounded-xl">
          <span className="text-[#4a5568] font-bold block mb-1 text-[10px] uppercase">INGESTION UTC</span>
          <span className="text-[#2d3436] font-medium">{custody.ingestion_timestamp_utc?.replace('T', ' ').substring(0, 19)}</span>
        </div>
        <div className="slot-recessed p-3.5 rounded-xl">
          <span className="text-[#4a5568] font-bold block mb-1 text-[10px] uppercase">FILE CHECKSUMS</span>
          <span className="text-[#4a5568] block truncate text-[10px] font-bold">MD5: {custody.md5 || 'N/A'}</span>
          <span className="text-[#4a5568] block truncate text-[10px]">Size: {custody.file_size_bytes ? `${custody.file_size_bytes} B` : 'N/A'}</span>
        </div>
      </div>
      ...
      {data.blockchain_receipt && (
        <div className="slot-recessed p-3.5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 text-xs">
          ...
          <div className="font-mono text-[11px] text-[#4a5568] truncate max-w-full sm:max-w-md">
            Txn: <span className="text-[#2d3436] font-bold">{data.blockchain_receipt.transaction_hash || "0x..."}</span>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs pt-1">
        ...
        <div className="slot-recessed p-4 space-y-2 flex flex-col justify-between">
          <div>
            <span className="text-[#4a5568] font-bold block uppercase tracking-wider text-[11px] mb-1 font-mono">
              Origin Infrastructure Tier
            </span>
            <div className="text-sm font-bold text-[#2d3436] flex items-center gap-2">
              <span className={`w-2.5 h-2.5 rounded-full ${infra.is_vpn_proxy ? 'bg-[#ff4757]' : (infra.is_cloud ? 'bg-[#f59e0b]' : 'bg-[#0ea5e9]')}`} />
              {infra.infra_type || "Standard ISP"}
            </div>
            <p className="text-[#4a5568] text-[11px] mt-1 leading-relaxed font-sans">{infra.details}</p>
          </div>
          {infra.ip && (
            <div className="pt-2 border-t border-[#babecc]/50 text-[11px] font-mono text-[#4a5568]">
              Analyzed IP: <span className="text-[#2d3436] font-bold">{infra.ip}</span>
            </div>
          )}
        </div>
        ...
        <div className="slot-recessed p-4 space-y-2 flex flex-col justify-between">
          <div>
            ...
            <div className="text-sm font-bold text-[#2d3436] flex items-center gap-1.5">
              {dns.is_resolvable ? (
                <>
            ...
            <p className="text-[#4a5568] text-[11px] mt-1 leading-relaxed font-sans">
              {dns.risk_indicators?.length > 0 ? dns.risk_indicators.join("; ") : "Valid DNS and MX routing records configured."}
            </p>
          </div>
          {dns.domain && (
            <div className="pt-2 border-t border-[#babecc]/50 text-[11px] font-mono text-[#4a5568] truncate">
              Domain: <span className="text-[#2d3436] font-bold">{dns.domain}</span>
            </div>
          )}
        </div>
        ...
        <div className="slot-recessed p-4 space-y-2 flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between mb-1">
              <span className="text-[#4a5568] font-bold block uppercase tracking-wider text-[11px] font-mono">
                WHOIS & Registrar Intel
              </span>
              {getDomainAgeBadge()}
            </div>
            
            <div className="text-sm font-bold text-[#2d3436] truncate">
              {whois.registrar || <span className="text-[#8896aa] italic font-normal">Registrar Unspecified</span>}
            </div>

            <div className="space-y-1 mt-2 text-[11px] text-[#2d3436]">
              {whois.creation_date && (
                <div className="flex items-center gap-1.5 text-[#4a5568]">
                  <Calendar className="w-3.5 h-3.5 text-[#7048e8] flex-shrink-0" />
                  <span>Created: <strong className="text-[#2d3436] font-mono">{whois.creation_date.substring(0, 10)}</strong></span>
                </div>
              )}
              {whois.is_privacy_protected !== undefined && (
                <div className="flex items-center gap-1.5">
                  <Lock className={`w-3.5 h-3.5 ${whois.is_privacy_protected ? 'text-[#f59e0b]' : 'text-[#8896aa]'} flex-shrink-0`} />
                  <span className="text-[#4a5568]">
                    Privacy: <strong className={whois.is_privacy_protected ? 'text-[#b45309]' : 'text-[#2d3436]'}>
          ...
          {whois.registrant_org && (
            <div className="pt-2 border-t border-[#babecc]/50 text-[11px] font-mono text-[#4a5568] truncate">
              Org: <span className="text-[#2d3436] font-bold">{whois.registrant_org}</span>
            </div>
          )}
        </div>
      ...
      {showPreviewModal && reportHtml && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="panel-chassis w-full max-w-4xl max-h-[90vh] flex flex-col shadow-2xl overflow-hidden border border-[#babecc]">
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-[#d1d9e6] bg-[#e0e5ec]">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <h3 className="text-sm font-bold text-[#2d3436] font-mono">FORENSIC INVESTIGATION REPORT PREVIEW</h3>
                  <p className="text-[10px] text-[#4a5568] font-mono">Evidence ID: {custody.evidence_id}</p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className="p-1.5 text-[#4a5568] hover:text-[#2d3436] rounded-lg transition-colors cursor-pointer ml-2"
                >
                  <X className="w-5 h-5" />
                </button>
              </div>
            ...
            <div className="px-6 py-3 bg-[#f0f2f5] border-b border-[#d1d9e6] flex flex-wrap items-center justify-between gap-3 text-xs">
              <div className="flex items-center gap-2">
                <span className="text-[#4a5568] font-bold text-[11px] font-mono">Classification:</span>
                <select
                  ...
                  className="bg-[#e0e5ec] border border-[#babecc] text-[#2d3436] text-[11px] rounded-lg px-2.5 py-1 focus:outline-none cursor-pointer font-mono font-bold"
                >
              ...
              <div className="flex items-center gap-2">
                <span className="text-[#4a5568] font-bold text-[11px] font-mono">Investigator:</span>
                <input
                  type="text"
                  value={investigator}
                  onChange={(e) => setInvestigator(e.target.value)}
                  onBlur={() => fetchReportHtml({ classification, investigator, agency })}
                  className="bg-[#e0e5ec] border border-[#babecc] text-[#2d3436] text-[11px] rounded-lg px-2.5 py-1 w-44 focus:outline-none font-mono font-bold"
                  placeholder="Analyst Name / Badge"
                />
              </div>

              <div className="flex items-center gap-2">
                <span className="text-[#4a5568] font-bold text-[11px] font-mono">Agency / Unit:</span>
                <input
                  type="text"
                  value={agency}
            ...
            <div className="flex-1 overflow-y-auto p-4 slot-recessed bg-[#d1d9e6]">
              <iframe
                title="Forensic Report Preview"
                srcDoc={reportHtml}
                className="w-full h-[65vh] rounded-xl border border-[#babecc] bg-white shadow-inner"
              />
            </div>
