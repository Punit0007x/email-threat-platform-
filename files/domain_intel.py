...
def get_dns_records(domain: str) -> tuple[list[str], list[str], bool, bool, list[str]]:
    ...
    try:
        for rdata in _dns_resolver.resolve(domain, "MX"):
            mx_hosts.append(str(rdata.exchange).rstrip("."))
    except Exception as exc:
        errors.append(f"MX lookup failed: {exc}")

    try:
        for rdata in _dns_resolver.resolve(domain, "NS"):
            ns_hosts.append(str(rdata.target).rstrip("."))
    except Exception as exc:
    ...
    try:
        for rdata in _dns_resolver.resolve(f"_dmarc.{domain}", "TXT"):
            txt = "".join(s.decode() if isinstance(s, bytes) else s for s in rdata.strings)
