...
GRAPH_DB_PATH = "data/attribution_graph.json"

_GLOBAL_GRAPH = None

def get_global_graph(namespace: str = "default") -> nx.Graph:
    """Gets the graph instance for the specific tenant/namespace."""
    # Since multi-tenant is requested: scope by namespace
    global _GLOBAL_GRAPH
    if _GLOBAL_GRAPH is None:
        _GLOBAL_GRAPH = {}
        
    if namespace not in _GLOBAL_GRAPH:
        graph_path = f"data/attribution_graph_{namespace}.json"
        graph = nx.Graph()
        if os.path.exists(graph_path):
            try:
                ...
                pass
        _GLOBAL_GRAPH[namespace] = graph
        
    return _GLOBAL_GRAPH[namespace]

def persist_graph(graph: nx.Graph, namespace: str = "default"):
    graph_path = f"data/attribution_graph_{namespace}.json"
...
def add_incident(graph: nx.Graph, incident: Incident) -> None:
    """
    Inserts a new incident into the graph, mapping out nodes for its shared
    infrastructure and linking them back to the incident.
    """
    incident_dict = {
        "incident_id": incident.incident_id,
        "from_address": incident.from_address,
        "from_domain": incident.from_domain,
        "originating_ip": incident.originating_ip,
        "asn": incident.asn,
        "reply_to_domain": incident.reply_to_domain,
        "subject": incident.subject,
        "fraud_score": incident.fraud_score,
        "timestamp": incident.timestamp.isoformat() if incident.timestamp else None,
    }
    
    graph.add_node(incident.incident_id, _type="incident", **incident_dict)
